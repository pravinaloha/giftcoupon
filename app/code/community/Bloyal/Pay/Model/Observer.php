...
class Bloyal_Pay_Model_Observer {
    ...
    public function hookIntoCheckoutOnePageSuccessAction($observer) {
        ...
        if (($paymentcode == 'bloyalgiftcard') || ($order->getGiftCardNo())) {

            try {
                if ($paymentcode == 'bloyalgiftcard') {
                ...
                $gcSQL = "UPDATE " . Mage::getConfig()->getTablePrefix() . "bloyal_gift_cards SET " . Mage::getConfig()->getTablePrefix() . "bloyal_gift_cards.gift_card_balance = " . $gcBalance . " WHERE " . Mage::getConfig()->getTablePrefix() . "bloyal_gift_cards.gift_card_number = '" . $gcNumber . "'";
                $dbWrite->query($gcSQL);
            } catch (Exception $e) {
                Mage::throwException($e->getMessage());
                exit;
            }
            Mage::log("Gift Card Number: " . $gcNumber . " Has a new Balance of: " . $gcBalance, 'loyalty.log');
            return $this;
        }
    }
    ...
    public function updatePaypalTotal($observer) {
        ...
        Mage::log("Gift Card Total sent to PayPal: " . $salesEntity->getGiftCardValue(), 'loyalty.log');
    }
